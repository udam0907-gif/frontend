...
| 형식 | 역할 | 우선순위 |
|------|------|----------|
| DOCX | 자동 출력 주력 — docxtpl `{{variable}}` 채우기 | 1순위 |
| XLSX | legacy/이행용 — 기존 field_map 셀 매핑 유지 | 2순위 |
| PDF/JPG/PNG | 첨부용 — passthrough_copy | 3순위 |

- 변수 스키마 정의: `backend/app/schemas/docx_schemas.py`
- 문서 타입별 정의 대상: quote, comparative_quote, transaction_statement, inspection_confirmation, expense_resolution
- 새 DOCX 템플릿 업로드 시: document_type 지정 → 해당 스키마 변수명으로 `{{variable}}` 삽입
...
- Document: docxtpl (DOCX 서식 채움, `{{variable}}` / `{% for %}` 블록)
