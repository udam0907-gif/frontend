...
class TemplateService:
    ...
    def validate_file(self, filename: str, content: bytes) -> None:
        if not validate_file_extension(filename, ALLOWED_TEMPLATE_EXTENSIONS):
            exts = " / ".join(sorted(ALLOWED_TEMPLATE_EXTENSIONS))
            raise TemplateError(
                f"허용되지 않는 파일 형식입니다. 업로드된 파일: {filename}. "
                f"허용 형식: {exts}"
            )
    ...
    def extract_placeholders(self, file_path: str) -> dict[str, Any]:
        """
        Extract {{placeholder}} patterns from DOCX files.
        Non-DOCX files (XLSX, PDF, images) are stored as-is with an empty field_map.
        """
        ext = Path(file_path).suffix.lower()
        if ext not in _DOCX_EXTS:
            logger.info("placeholders_skipped", file=file_path, ext=ext)
            return {}
        ...
        logger.info("placeholders_extracted", file=file_path, count=len(placeholders))
        return field_map
