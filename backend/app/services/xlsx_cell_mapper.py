...
1. cell_map에는 「동적 입력 영역」만 포함한다.
   동적 입력 영역: 매 출력마다 사용자 입력으로 달라지는 값 (수신처/공급받는자 전체 정보, 작성일자, 품목, 수량, 단가, 합계 등).
...
3. 공급받는자(우리 회사) 영역: 견적서/거래명세서의 「공급받는자」 박스 안에 사업자등록번호·상호·대표자·주소·업태·종목을
   입력하는 빈 셀들을 recipient_business_number, recipient_company_name, recipient_representative,
   recipient_address, recipient_business_type, recipient_business_item으로 반드시 찾을 것.
   공급받는자 영역이 없는 양식이면 null 허용.
4. 셀 주소는 반드시 "B5" 형식(열문자+행번호). 병합 셀은 반드시 좌상단(앵커) 셀 좌표로 표현.
5. 순수 JSON만 반환. 다른 텍스트 절대 불가."""
    ...
    async def analyze(self, file_path: str) -> dict[str, Any]:
        ...
        try:
            ...
            user_msg = (
                ...
                "- recipient_name: '귀하', '귀중', '수신처', '貴中' 등 수신자 상호가 입력되는 셀. 라벨 옆/아래 빈 셀.\n"
                ...
                "【공급받는자(우리 회사) 영역 — 동적 입력 필드 (존재하면 반드시 포함)】\n"
                "견적서/거래명세서의 「공급받는자」 박스 안에 우리 회사 정보를 입력하는 빈 셀들:\n"
                "- recipient_business_number: 공급받는자 사업자등록번호 입력 셀\n"
                "- recipient_company_name: 공급받는자 상호/법인명 입력 셀\n"
                "- recipient_representative: 공급받는자 대표자명 입력 셀\n"
                "- recipient_address: 공급받는자 주소 입력 셀\n"
                "- recipient_business_type: 공급받는자 업태 입력 셀\n"
                "- recipient_business_item: 공급받는자 종목 입력 셀\n"
                "  → 이 6개 키는 좌우 분리든 위아래 분리든 양식 구조에 관계없이 모두 찾을 것.\n"
                "  → 공급받는자 박스가 없는 양식(단순 귀하 라인만)이면 null 허용.\n\n"
                "【추가 필드 — 동적 입력이면 포함】\n"
                "- doc_number: 문서번호/견적번호가 입력되는 셀.\n"
                "- total_amount_korean: 합계 한글 표기 셀 (있으면 포함).\n"
                "- note / remark: 비고란이 입력되는 셀 (있으면 포함).\n"
                "- total_tax: 부가세 합계 금액 셀 (세액합계, 부가세계, tax total).\n"
                "- total_supply: 공급가액 합계 셀 (공급가합계, supply total, VAT 제외).\n\n"
                ...
                '"columns": {"code": "<열문자>", "item_name": "<열문자>", "spec": "<열문자>", "unit": "<열문자>", '
                '"quantity": "<열문자>", "unit_price": "<열문자>", "amount": "<열문자>", "tax": "<열문자>"}, '
                '"max_rows": <품목 최대 입력 가능 행 수(정수)>}}\n'
                "columns 안의 열문자가 없으면 null. code=품번/코드, tax=행별세액. 품목 표가 없으면 '_meta'를 생략.\n\n"
                "【작성일자 처리 — 분리 셀 지원】\n"
                ...
                '  "cell_map": {\n'
                '    "recipient_name": "B4",\n'
                '    "recipient_business_number": "B5",\n'
                '    "recipient_company_name": "D5",\n'
                '    "recipient_representative": "F5",\n'
                '    "recipient_address": "B6",\n'
                '    "recipient_business_type": "B7",\n'
                '    "recipient_business_item": "D7",\n'
                '    "issue_date": "A3",\n'
                '    "item_name": "A17",\n'
                '    "spec": "B17",\n'
                '    "unit": "C17",\n'
                ...
                '    "total_amount": "F30",\n'
                '    "doc_number": "G3",\n'
                '    "_meta": {"items_table": {"start_row": 17, "columns": {"code": null, "item_name": "A", "spec": "B", "unit": "C", "quantity": "D", "unit_price": "E", "amount": "F", "tax": null}, "max_rows": 10}}\n'
                "  }\n"
                "}\n"
