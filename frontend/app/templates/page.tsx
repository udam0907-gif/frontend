  ...
  return (
    <div className="border-t border-gray-100 bg-gray-50 px-4 py-4">
      <p className="text-xs text-gray-500 mb-3">
        각 필드가 XLSX의 어느 셀에 해당하는지 입력하세요. 예: <code className="bg-gray-200 px-1 rounded">B3</code>, <code className="bg-gray-200 px-1 rounded">E10</code>
      </p>
      <div className="mb-3 p-2 bg-blue-50 border border-blue-100 rounded text-xs text-blue-700 space-y-0.5">
        ...
        <p>· Excel에서 셀 선택 후 수식입력줄 좌측 이름상자로 주소 확인</p>
      </div>
              ...
              <tr key={field.key} className="border-b border-gray-100 last:border-0">
                ...
                <td className="py-1.5">
                  <Input
                    ...
                    placeholder="예: B3"
                    className={`h-7 text-xs w-24 font-mono uppercase ${
                      errors[field.key] ? "border-red-400 focus-visible:ring-red-400" : ""
                    }`}
                    maxLength={6}
                  />
                  {errors[field.key] && (
                    <p className="text-red-500 text-xs mt-0.5">잘못된 형식</p>
                  )}
                </td>
              </tr>
            ))}
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {mappedCount > 0 ? (
                ...
                <Badge className="text-xs bg-amber-50 text-amber-600">
                  매핑 미설정
                </Badge>
              )}
              {!tmpl.is_active && (
                <Badge className="text-xs bg-red-50 text-red-500">비활성</Badge>
              )}
            </div>
    ...
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-0.5">
          XLSX 서식 파일을 업로드하고 필드별 셀 주소를 매핑하세요.
        </p>
      </div>
